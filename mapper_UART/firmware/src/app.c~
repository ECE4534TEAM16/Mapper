...
//(last turn, distance since last turn, chars representing intersection type)
...
void APP_Initialize( void )
{
    ...
    MsgQueue_Mapper_Directions = xQueueCreate(APP_BUFFER_SIZE, 5*sizeof(char));
    if( MsgQueue_Mapper_Directions == 0 )
    {
        //BAD
        //fatal error
    }
    
    MsgQueue_Mapper_Start = xQueueCreate(APP_BUFFER_SIZE, sizeof( char ));
    if( MsgQueue_Mapper_Directions == 0 )
    {
        //BAD
        //fatal error
    ...
    setError("2100f");
    setError("2100l");
    xQueueSendToBack(MsgQueue_Mapper_Directions, (void *) &inst, APP_NUMBER_OF_TICKS);
...
{    
    if(APP_TEST)
    {
        xQueueSendToBack(MsgQueue_Mapper_Directions, error, APP_NUMBER_OF_TICKS);
    }
    else
    {
...
void check_Start()
{
    if(appData.buffer[0] == 'S')
    {
        appData.start = true;
        //set boolean here to start rover
    }
}
...
void APP_Tasks( void )
{
    ...
    switch ( appData.currentState )
    {
        ...
        {
            //char mapper_buffer[5];
            appData.usartStatus = DRV_USART_ClientStatus( appData.usartHandle );

            if(uxQueueMessagesWaiting(MsgQueue_Mapper_Directions) > 0)
            {
                ...
                if ( appData.usartStatus == DRV_USART_CLIENT_STATUS_READY )
                {
                    if(xQueueReceive(MsgQueue_Mapper_Directions, &temp, APP_NUMBER_OF_TICKS))
                    {
                        strcpy(test, "\r\n");
                        strcat(test, temp );
                        appData.bufferSize = min(APP_BUFFER_SIZE,
...
void App_GetNextTaskState(uint32_t appState)
{
    switch ( appState )
    {
        ...
        case APP_DRV_MSG_WRITE:
            ...
            appData.currentState = APP_USR_MSG_READ; //needs to wait here until start signal is sent
